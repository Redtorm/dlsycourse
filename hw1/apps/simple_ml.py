...
def parse_mnist(image_filename, label_filename):
    ...
    magic_num = struct.unpack('>i', image_data[0:4])
    image_num = struct.unpack('>i', image_data[4:8])
    row_num, col_num = struct.unpack('>ii', image_data[8:16])
    piexl_num = row_num * col_num
    X = (np.frombuffer(image_data[16:], dtype=np.uint8) / 255).reshape((image_num[0], piexl_num)).astype(np.float32)

    label_num = struct.unpack('>i', label_data[4:8])
    y = np.frombuffer(label_data[8 : 8 + label_num[0]], dtype=np.uint8)
